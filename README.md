...
| `packedStorageUnpack`  | `SimpleBitStorage` / `PalettedContainer` | Vectorizes both directions of palette bit-packing — `long[]` → `int[]` unpacking and `int[]` → `long[]` packing — hit on every chunk save and every chunk load.                                                                                                                                             |
...
| Mod                                        | Status                                                                                                                                                                                                                                                              |
|--------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Lithium**                                | Compatible. Lithium `@Overwrite`s `PalettedContainer.pack()` with a fused implementation that never calls `SimpleBitStorage.unpack()`, so VectorX's hook is simply **inert** there — not broken. The chunk-*load* path is untouched by Lithium and always benefits. |
