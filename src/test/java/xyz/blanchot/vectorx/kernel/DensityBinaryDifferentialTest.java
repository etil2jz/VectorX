...
class DensityBinaryDifferentialTest {
    ...
    void constMatchesRealMinecraftSampleVolume(DensityBinaryOp op) {
        ...
        for (float operand : new float[]{0.0F, -0.0F, 1.0F, -1.0F, 0.37F, Float.NaN}) {
            ...
            assertArrayEquals(expected, scalarOut, () -> "scalar mismatch vs real const sampler, op=" + op + " operand=" + operand);
            assertArrayEquals(expected, vectorOut, () -> "vector mismatch vs real const sampler, op=" + op + " operand=" + operand);
        }
    }
    ...
    void vanillaDisagreesWithItselfOnZeroTimesInfinity() {
        ...
        BinaryFunction.MulSampler sampler = new BinaryFunction.MulSampler(new ArraySampler(left), new ArraySampler(right));
        ...
        assertEquals(0.0F, viaValue, "sampleValue short-circuits a zero left operand to 0.0f -- if this ever fails, Mojang made the two paths consistent and the kernels should be revisited");
    ...
    void vanillaDisagreesWithItselfOnSignedZero() {
        ...
        BinaryFunction.ConstMinSampler sampler = new BinaryFunction.ConstMinSampler(new ArraySampler(values), -0.0F);

        float[] viaVolume = vanilla(sampler, values.length);
        float viaValue = sampler.sampleValue(SamplerContext.EMPTY_UNCACHED, 0, 0, 0);

        assertEquals(Float.floatToIntBits(0.0F), Float.floatToIntBits(viaVolume[0]), "sampleVolume's conditional write should keep +0.0f");
        assertEquals(Float.floatToIntBits(-0.0F), Float.floatToIntBits(viaValue), "sampleValue's Math.min should return -0.0f -- if this ever fails, Mojang made the two paths consistent and the kernels should be revisited");
        ...
        assertEquals(Float.floatToIntBits(0.0F), Float.floatToIntBits(scalarOut[0]), "scalar kernel must follow sampleVolume, not Math.min");
        assertEquals(Float.floatToIntBits(0.0F), Float.floatToIntBits(vectorOut[0]), "vector kernel must follow sampleVolume, not Math.min");
    }
