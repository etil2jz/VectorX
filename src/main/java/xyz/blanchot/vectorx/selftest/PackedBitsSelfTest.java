...
public final class PackedBitsSelfTest {
    ...
    private static final long SEED = 0x504B4249_5453L;
    ...
    private static void checkPack(int bits, long mask, int valuesPerLong, PackedBitsKernels scalar, PackedBitsKernels vector) {
        ...
        for (int size : SIZES) {
            ...
            for (int i = 0; i < requiredLongs; i++) {
                if (scalarOut[i] != vectorOut[i]) {
                    throw new SelfTestFailure("pack bits=" + bits + " size=" + size + " wordIndex=" + i + ": scalar=" + Long.toHexString(scalarOut[i]) + " vector=" + Long.toHexString(vectorOut[i]));
                }
            }
        }
    }

    private static void checkPattern(int bits, int valuesPerLong, long fillValue, PackedBitsKernels scalar, PackedBitsKernels vector) {
    ...
    private static void compare(int bits, int size, long[] data, PackedBitsKernels scalar, PackedBitsKernels vector, String label) {
        ...
        for (int i = 0; i < size; i++) {
            if (scalarOut[i] != vectorOut[i]) {
                throw new SelfTestFailure("bits=" + bits + " size=" + size + " (" + label + ") index=" + i + ": scalar=" + scalarOut[i] + " vector=" + vectorOut[i]);
            }
        }
    }
