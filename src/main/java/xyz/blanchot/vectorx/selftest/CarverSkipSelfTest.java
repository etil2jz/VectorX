...
public final class CarverSkipSelfTest {
    ...
    private static void checkRandom(int n, Random random, CarverSkipKernels scalar, CarverSkipKernels vector) {
        ...
        compare(n, minGenY, minY, maxY, horizSum, y, verticalRadius, widthFactorPerHeight, scalar, vector, "random n=" + n);
    }
    ...
    private static void checkBoundary(CarverSkipKernels scalar, CarverSkipKernels vector) {
        ...
        compare(n, minGenY, minY, n, horizSum, y, verticalRadius, widthFactorPerHeight, scalar, vector, "boundary");
    }

    private static void compare(int n, int minGenY, int minY, int maxY, double horizSum, double y, double verticalRadius, float[] widthFactorPerHeight, CarverSkipKernels scalar, CarverSkipKernels vector, String label) {
        ...
        for (int i = 0; i < n; i++) {
            if (scalarOut[i] != vectorOut[i]) {
                throw new SelfTestFailure("(" + label + ") index=" + i + ": scalar=" + scalarOut[i] + " vector=" + vectorOut[i]);
            }
        }
    }
