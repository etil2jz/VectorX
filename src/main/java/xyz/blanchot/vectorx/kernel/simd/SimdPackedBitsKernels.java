...
public final class SimdPackedBitsKernels implements PackedBitsKernels, SelfDescribing {
    ...
    private static final int VALUES_PER_LONG_4 = 16;
    ...
    private static final boolean UNPACK4_SUPPORTED = LANES_4 > 0 && LANES_4 <= VALUES_PER_LONG_4 && VALUES_PER_LONG_4 % LANES_4 == 0;
    private static final int PASSES_PER_LONG_4 = UNPACK4_SUPPORTED ? VALUES_PER_LONG_4 / LANES_4 : 0;
    private static final VectorSpecies<Integer> INT_SPECIES_4 = UNPACK4_SUPPORTED ? VectorSpecies.of(int.class, VectorShape.forBitSize(LANES_4 * Integer.SIZE)) : null;
    private static final LongVector MASK_VEC_4 = UNPACK4_SUPPORTED ? LongVector.broadcast(LONG_SPECIES_4, 0xFL) : null;
    ...
    private void pack4(int[] values, int size, long[] output) {
        ...
        for (; li < fullLongs; li++) {
            ...
            for (int pass = 0; pass < PASSES_PER_LONG_4; pass++) {
                ...
                LongVector shifted = widened.lanewise(VectorOperators.AND, MASK_VEC_4).lanewise(VectorOperators.LSHL, SHIFT_BASE_4[pass]);
    ...
    private void unpack5(long[] data, int size, int[] output) {
        ...
        for (; li < fullLongs; li++) {
            ...
            for (int chunk = 0; chunk < CHUNKS_PER_LONG_5; chunk++) {
                LongVector masked = broadcast.lanewise(VectorOperators.LSHR, SHIFT_BASE_5[chunk]).lanewise(VectorOperators.AND, MASK_VEC_5);
    ...
    private void unpack6(long[] data, int size, int[] output) {
        ...
        for (; li < fullLongs; li++) {
            ...
            for (int chunk = 0; chunk < CHUNKS_PER_LONG_6; chunk++) {
                LongVector masked = broadcast.lanewise(VectorOperators.LSHR, SHIFT_BASE_6[chunk]).lanewise(VectorOperators.AND, MASK_VEC_6);
                IntVector iv = (IntVector) masked.convertShape(VectorOperators.L2I, INT_SPECIES_4, 0);
                int outBase = base + chunk * LANES_4;
    ...
    private void unpack7(long[] data, int size, int[] output) {
        ...
        for (; li < fullLongs; li++) {
            ...
            for (int chunk = 0; chunk < CHUNKS_PER_LONG_7; chunk++) {
                LongVector masked = broadcast.lanewise(VectorOperators.LSHR, SHIFT_BASE_7[chunk]).lanewise(VectorOperators.AND, MASK_VEC_7);
                IntVector iv = (IntVector) masked.convertShape(VectorOperators.L2I, INT_SPECIES_4, 0);
                int outBase = base + chunk * LANES_4;
    ...
    private void unpack15(long[] data, int size, int[] output) {
        ...
        for (; li < fullLongs; li++) {
            ...
            for (int chunk = 0; chunk < CHUNKS_PER_LONG_15; chunk++) {
                LongVector masked = broadcast.lanewise(VectorOperators.LSHR, SHIFT_BASE_15[chunk]).lanewise(VectorOperators.AND, MASK_VEC_15);
                IntVector iv = (IntVector) masked.convertShape(VectorOperators.L2I, INT_SPECIES_4, 0);
                int outBase = base + chunk * LANES_4;
    ...
    private void unpack4(long[] data, int size, int[] output) {
        ...
        for (; li < fullLongs; li++) {
            ...
            int base = li * VALUES_PER_LONG_4;
            for (int pass = 0; pass < PASSES_PER_LONG_4; pass++) {
                LongVector masked = broadcast.lanewise(VectorOperators.LSHR, SHIFT_BASE_4[pass]).lanewise(VectorOperators.AND, MASK_VEC_4);
                IntVector iv = (IntVector) masked.convertShape(VectorOperators.L2I, INT_SPECIES_4, 0);
    ...
    public String describe() {
        return "int=" + INT_SPECIES + ", byte=" + BYTE_SPECIES + " (" + BYTE_TO_INT_PARTS + " parts/int), short=" + SHORT_SPECIES + " (" + SHORT_TO_INT_PARTS + " parts/int), long4=" + LONG_SPECIES_4 + " (" + PASSES_PER_LONG_4 + " passes/long, supported=" + UNPACK4_SUPPORTED + "); unpack vectorizes bits in {4,5,6,7,8,15,16,32}, pack vectorizes bits in {4}";
    }
}
