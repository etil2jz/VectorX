...
import xyz.blanchot.vectorx.selftest.DensityMapSelfTest;
...
public final class DensityMapDispatcher {
    ...
    private final DensityMapKernels backend;
    private final boolean vector;
    private final String disableReason;

    public DensityMapDispatcher(VectorXConfig config, VectorXLog log) {
        this(config, log, DensityMapDispatcher.class.getClassLoader());
    }

    DensityMapDispatcher(VectorXConfig config, VectorXLog log, ClassLoader loader) {
        Objects.requireNonNull(config, "config");
        Objects.requireNonNull(log, "log");

        String reason = globalDisableReason(config);
        if (reason == null) {
            DensityMapKernels candidate = tryLoadVectorBackend(loader, log);
            if (candidate == null) {
                ...
                if (mode == KernelMode.SCALAR || mode == KernelMode.OFF) {
                    ...
                    DensityMapSelfTest.Result result = DensityMapSelfTest.run(ScalarDensityMapKernels.INSTANCE, candidate);
                    if (!result.passed()) {
        ...
        this.backend = ScalarDensityMapKernels.INSTANCE;
        this.vector = false;
    ...
    private static DensityMapKernels tryLoadVectorBackend(ClassLoader loader, VectorXLog log) {
        try {
            Class<?> simdClass = Class.forName(SIMD_CLASS_NAME, true, loader);
            Object instance = simdClass.getField(SIMD_INSTANCE_FIELD).get(null);
            return (DensityMapKernels) instance;
    ...
    public DensityMapKernels backend() {
        return backend;
    }
